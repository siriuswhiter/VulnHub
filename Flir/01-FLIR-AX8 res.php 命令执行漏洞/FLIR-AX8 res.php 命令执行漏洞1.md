...
在 www 目录下 res.php 程序接收 POST 方法请求，若存在 get 参数，并且提供一个 resource 参数，resource 不是 .rtp.hflip 和 .rtp.vflip 时，程序会将 resource  的值与 LD_LIBRARY_PATH=/FLIR/usr/lib /FLIR/usr/bin/rls -o 拼接执行，若 resource  包含命令截断符，会造成命令注入。
